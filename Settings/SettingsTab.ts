...
export class SettingsTab extends obsidian.PluginSettingTab {
	...
	display(): void {
		 ...
		 */
		const tutorialContainer = containerEl.createEl("div", {
			cls: "moving_rules_container",
		});
		const header = tutorialContainer.createEl("div", { cls: "rule_header" });
		header.createEl("h2", { text: "Tutorial" });
		/**
		 * Rule description and tutorial
		 */
		const description = tutorialContainer.createEl("div");
		...
		description.createSpan({
			text: "To move files containing the word 'Scroll' and a number divided to space -> to a folder named 'Scrolls' under the subfolder of the scroll number: ",
		});
		const example2 = description.createDiv({ cls: "rule margig_right" });
		example2.createEl("span", {
			text: "Scroll (\\d+)",
			cls: "rule_title",
		});
		example2.createEl("span", {
		...
		movingRulesContainer.createEl("h2", { text: "Regex moving rules", cls: "rule_header" });

		const ruleList = movingRulesContainer.createEl("div", { cls: "rule_list" });
